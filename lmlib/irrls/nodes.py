...
class Section():
    """Defines the (repeating) part of a factor graph propagating messages from time index k-1 to k"""
    ...
    def add_block(self, block, label=None):
        """Adds a new block to the end of the previous blocks"""
        self._block_entries.append((label, block))

    def optimize(self, max_itr, msg_fwrd_x0=None, msg_bwrd_x0=None):
        """Optimizes the factor graph defined by the section by (iterative) message passing"""
...
class Block():
    ...
    Each node is connected to one or several branches.
    ...
    def marginal_MV(fMV, dXW):
        """
       Computes marginals on single branch based on forward plus dual message on branch.
...
class MBF():
    ...
    class Block_Marginals():
        ...
        def __init__(self, K, N, X_dXWs=None, Y_fMVs=None):
            ...
            Parameters
            ----------
                X_dXWs : Memory to buffer dual messages or 'None'. If buffer is 'None', a new buffer will be allocated. Default: 'None'
                Y_fMVs : Memory to buffer backward messages or 'None'. If buffer is 'None', a new buffer will be allocated. Default: 'None'
            """
    ...
    class Block_System_A():
        ...
        def __init__(self, A):
            ...
            assert A.shape[0] == A.shape[1], "Matrix A must be a square matrix, but A of shape=" + str(
                A.shape) + " was given instead."
        ...
        def pass_forward(self, X_fMV, k):
            ...
            assert A.shape == X_fMV[1].shape, f"Matrix A is of dimension " + str(
                X_fMV[0].shape) + " instead of (" + str(X_fMV[1].shape) + "."
        ...
        def pass_backward(self, Y_dXW, k):
            ...
            assert A.shape == Y_dXW[1].shape, f"Matrix A is of dimension " + str(A.shape) + " instead of (" + str(Y_dXW[1].shape) + "."
    ...
    class Block_Input_NUV(Block_Input_BU):
        ...
        def __init__(self, B, sigmaU2, K=None, fU_mV=None, mU_VU=None, border_suppression=False):
            ...
            if fU_mV is None:
                self._fU_mV = Messages_MV(K, M, sigma2=sigmaU2, m=0,
                                          border_suppression=border_suppression)  # forward MV message on X
            else:
...
class Messages_MV():
    ...
    def __setitem__(self, k, value):
       ...
       """
        ...
        assert value[0].shape == (self._N,), f"Mean value dimension of " + str(
            value[0].shape) + " does not match required mean array element dimension of (" + str(self._N) + ",)."
        assert value[1].shape == (self._N, self._N), f"Variance value dimension of " + str(
            value[0].shape) + " does not match required variance array element dimension of (" + str(
            self._N) + "," + str(self._N) + ")."
