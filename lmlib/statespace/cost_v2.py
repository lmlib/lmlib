...
class CostSegment(BaseCost, BaseCost1d, ABC):
    ...
    def window(self, thd=1e-6):
        """
        Returns Segments Window

        See also :func:`Segment.window`

        Parameters
        ----------
        thd : float, optional
            Threshold for infinite segments
        """
        return  self.segment.window(thd)

    def trajectory(self, xs, thd=1e-6):
        ab_range = window_range(self.segment, thd)
        trajectories = np.empty((len(xs),), dtype=tuple)
        for i, x in enumerate(xs):
            trajectories[i] = ab_range, self.alssm.trajectory(x, ab_range)
        return trajectories
...
class CompositeCost(BaseCost, BaseCost1d, ABC):
    ...
    @property
    def segments(self):
        """Iterable : List/Tuple of Segment"""
