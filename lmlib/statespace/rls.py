...
class NDRLSAlssm:
    ...
    def filter(self, y, v=None, dim_order=None)-> None:
        ...
        if self.steady_state:
            ...
            # first dimension
            xi_prev = self._rls_1d__xi_q_recursion(q, y_MC, v, dim_order[0])

            # n-dimensions
            for nd_dim in dim_order[1:]:
                xi_prev = self._rls_nd__xi_q_asterisk_l_recursion(xi_prev, q, y_MC, v, nd_dim)
    ...
    def _rls_1d__xi_q_recursion(self, q, y, v, model_dimension):
        ...
        for segment, f, beta in zip(cost_model.segments, cost_model.F.T, betas):
            ...
            # backend recursion
            _xi_q_recursion(xi_curr, q, alssm, segment, _y, v, beta, self.backend, self.filter_form)

        return np.moveaxis(xi_curr, 0, model_dimension)

    def _rls_nd__xi_q_asterisk_l_recursion(self, xi_prev, q, y, v, model_dimension):
        cost_model = self.nd_cost_model[model_dimension]
        betas = self.nd_betas[model_dimension]
        N = cost_model.get_model_order()
    ...
    def filter_minimize_yhat(self, y, v=None, dim_order=None, H=None, h=None, alssm_weights=None, c0s=None):
        ...
        return self.nd_cost_model.eval_alssm_output(xs, alssm_weights=alssm_weights, c0s=c0s)
...
class RLSAlssm(NDRLSAlssm):
    ...
    #
    #     if return_constrains:
    #         v, H, h = super().minimize_v(H=H, h=h, return_constrains=return_constrains)
    #         return v, H, h
    #     vs = super().minimize_v(H=H, h=h, return_constrains=False)
    #     if np.ndim(vs) == 2:
    #         return vs
    #     elif np.ndim(vs) >= 3:
    #         return np.moveaxis(vs, -1, 1)
    #     else:
    #         raise ValueError("vs wrong dimension. \n")
