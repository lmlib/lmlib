...
IRLS_VW msg_X = IRLS_VW(K, N)
IRLS_VW msg_U = IRLS_Sigma(K, SZ_B[1])
...
def fw_input_BNU( mV_A, B, sigmaU2):
       (mF_A, VF_A) = mV_A
       
       # "AX+B@U=AX_" Node       
       mF_A_ = mF_A + B@0 # II.1, III.1
       VF_A_ = VF_A + B@sigmaU2@BT  # II.2, III.2     
       return (mF_A_, VF_A_)
       
def  fw_output_YZ(mF, VF, y, C, sigmaZ2):
       # "Y_+Z=Y" Node
       mB_Y_[k] = y
       VB_Y_[k] = sigmaZ2

       # "C=" Node
       G = np.linalg.inv(sigmaZ2 + C@VF@CT)  #  V.3
       mF_ = mF+VF@CT@G@(y - C@mF) #  V.1
       VF_ = VF-VF@CT@G@C@VF #  V.2    
       return (mF_, VF_)



def  bw_input_YZ(XI_X, W_X, y, C, sigmaZ2):
...
for ii in range(0,I_MAX):
    ...
    # 1) Forward Kalman
    for k in range(1,K):
       (mF_AX[k], VF_AX[k]) = fw_pass_A( (mF_X[k], VF_X[k]), A)
       (mF_AX_[k], VF_AX_[k]) = fw_input_BNU( (mF_AX[k], VF_AX[k]) , B, sigmaU2, m=0)
       (mF_X[k], VF_X[k]) = fw_output_YZ(mF_AX_[k], VF_AX_[k], y[k], C, sigmaZ2)

    # 2) Backward
    for k in range(K-1,0,-1):
       (XI_AX_[k], W_AX_[k]) = bw_input_YZ(XI_X[k], W_X[k], y[k], C, sigmaZ2)
       (XI_X[k], W_X[k]) = bw_input_A( (m, V) , A)            

    # 4) Posterior Mean (marginals)
    for k in range(K):
        # State X estimate
        (m_X, V_X) = marginal( (mF_X[k], VF_X[k]) , (XI_X[k], W_X[k]) ) 
        (m_U[k], V_U[k]) = marginal( (mF_U[k], VF_U[k]) , (XI_U[k], W_U[k]) ) 
