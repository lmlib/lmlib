...
K = 1000  # number of samples (length of test signal)
y = np.column_stack([gen_rand_walk(K, seed=12345678), gen_rand_walk(K, seed=314159865)])
L = y.shape[1] # output dimension
u = np.zeros((K, 2))
...
sigmaU2 = 1.0 * np.eye(M)# initial value for Sigma U^2
sigmaZ2 = 100.0 * np.eye(L) # initial value for Sigma Z^2
sigmaZ2 = sigmaZ2+np.rot90(np.eye(M))*0
...
m_U = np.zeros([K, M])  # Posterior variance at U
V_U = np.zeros([K, M, M])  # Posterior variance at X
...
inv_sigmaZ2 = np.linalg.inv(sigmaZ2)
...
## -- Plotting ---
ks = np.arange(K)
fig, axs = plt.subplots(6, 1, figsize=(8, 4), gridspec_kw={'height_ratios': [2, 1, 1, 2, 1, 1], 'hspace': 0.1}, sharex='all')

# Remove horizontal space between axes
fig.tight_layout()

axs[0].plot(ks, y[:, 0], color="gray", linestyle="-", lw=1.0, label='$y_0$ (Observations)')
axs[0].plot(ks, (C@m_X.T)[0], color="b", lw=1.25, label='posterior $m(X)$')
axs[0].legend(loc='upper right')


axs[1].plot(ks, m_U[:, 0], 'b', label='$u$ (m_U)')
axs[1].axhline(y = 0, color = 'k', linestyle = '-', linewidth=0.5)
axs[1].set(ylabel='Input Mean')
axs[1].legend(loc='upper right')

axs[2].plot(ks, VF_U[:,0,0], color="black", lw=0.75, label='$\sigma_U^2$ (sigmaU2)')
axs[2].set(ylabel='Input Var.')
axs[2].legend(loc='upper right')



axs[3].plot(ks, y[:, 1], color="gray", linestyle="-", lw=1.0, label='$y_0$ (Observations)')
axs[3].plot(ks, (C@m_X.T)[1], color="b", lw=1.25, label='posterior $m(X)$')
axs[3].legend(loc='upper right')


axs[4].plot(ks, m_U[:, 1], 'b', label='$u$ (m_U)')
axs[4].axhline(y = 0, color = 'k', linestyle = '-', linewidth=0.5)
axs[4].set(ylabel='Input Mean')
axs[4].legend(loc='upper right')

axs[5].plot(ks, VF_U[:,1,1], color="black", lw=0.75, label='$\sigma_U^2$ (sigmaU2)')
axs[5].set(ylabel='Input Var.')
