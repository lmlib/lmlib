...
segment_k, trajs_tmpl = cost.trajectories(alphas)[0][0][0]
...
for s, offset in enumerate(offset_channels):
    segment_k, trajs_tmpl = cost.trajectories(alphas)[0][s][0]
    ax1.plot(segment_k, trajs_tmpl + offset, lw=1, c='r', label=r'$\alpha^T z^q$')
...
for s, offset in enumerate(offset_channels):
    ax2.plot(t, trajs_obs_ext[s] + offset, lw=0.8, ls='--', c='b')
    ax2.plot(t, trajs_obs[s] + offset, lw=1, c='b')
ax2.legend([Line2D([0], [0], color='b', lw=1)], [r'$\beta_k^T z^q$'], loc=1, fontsize=8)

ax3.plot(t, y + offset_channels, lw=1, c='grey')
for s, offset in enumerate(offset_channels):
    ax3.plot(t, trajs_obs_ext[s]  + offset, lw=0.8, ls='--', c='b')
    ax3.plot(t, trajs_obs[s]  + offset, lw=1, c='b')
    ax3.plot(t, trajs_tmpl_hat[s]  + offset, lw=1, c='r', label=r'$\alpha^T z^q$')
