...
Each ALSSM cost term from `lmlib.statespace.cost` is weighted by its own window (or window weighting function).
This example demonstrates the application of the basic exponentially decaying window of finite and infinite support.
A more complex, symmetric windows (cf. `lmlib.statespace.cost.Segment`) is generated by composing multiple
exponential windows.
...
# Defines the minimum window weight (threshold) that is displayed as a non-zero value.
# only relevant for exponentially decaying windows with infinite support, as these never reach exactly 0
display_thd = 0.1
# display without window thresholding
wins = lm.map_windows(cost.windows([0, 1, 2, 3]), ks, K, merge_ks=True,  merge_seg=False)
# display with window thresholding
wins_all_no_thd = lm.map_windows(cost.windows([0, 1, 2, 3], display_thd), ks, K, merge_ks=True, merge_seg=True)
# display without window thresholding
wins_all = lm.map_windows(cost.windows([0, 1, 2, 3]), ks, K, merge_ks=True, merge_seg=True)
