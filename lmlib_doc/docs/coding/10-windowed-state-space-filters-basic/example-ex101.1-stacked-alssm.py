...
Note that `lmlib.statespace.model.Alssm.dump_tree()` returns the internal structure of the stacked LSSM and helps debugging.

Note: replacing `lmlib.statespace.model.AlssmStacked()` by
`lmlib.statespace.model.AlssmSum()` generates a summed (instead of a stacked) output.
